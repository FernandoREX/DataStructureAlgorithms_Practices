...
void Len(Hash_table* map)
{
    fprintf(stderr, "Elementos en la tabla: %ld\n", map->len );
}

void LoadFactor(Hash_table* tab)
{
    float loadfactor = tab->len / tab->size;
    printf("Elementos en la tabla: %f\n", loadfactor);
}
...
// Driver program 
 int main()
{
   Product productos[ MAX_PRODUCTS ] =   //ingresar 9 productos 
   {
      { 1000, "Gansito", 9.0 },     // [0]
      { 1400, "Crema", 16.5 },      // [1]
      { 3000, "Arroz", 28.5 },      // ...

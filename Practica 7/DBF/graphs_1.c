...
void Vertex_SetFinish_time( Vertex* v, size_t time )
{
    v->finish_time = time;
}

size_t Vertex_GetDiscovery_time(Vertex* v)
{
    return v->discovery_time;
}

size_t Vertex_GetFinish_time(Vertex* v)
{
...
Stack* Stack_New( size_t size )
{
   ...
   // Aquí quise usar malloc en lugar de calloc:
...
{
   DBG_PRINT( "dfs_rec()::Visiting vertex: %ld\n", Graph_GetIndexFromVertex( g, v ) );
   ...
   DBG_PRINT( "dfs_rec()::Dead end for vertex: %ld\n", Graph_GetIndexFromVertex( g, v ) );
}

// implementa el ordenamiento topológico:
static size_t dfs_recursive_topol( 
      Graph* g,     // pasamos el grafo para usar las funciones de su API
      Vertex* v,    // es el vértice d
      size_t time)  // es el tiempo de su predecesor
{
    ...
    printf("--- DISCOVERY TIME(KEY = %d): %ld ---\n",Vertex_GetKey(v), Vertex_GetDiscovery_time(v));
    ...
    for (Vertex_Start(v); !Vertex_End(v); Vertex_Next(v)) {
        ...
        //----------------Realiza una conversion de indice a vertice------------------

        size_t v_as_idx = Vertex_Get(v);
        // obtiene el indice del vecino en una lista de vertices

        Vertex* w = Graph_GetVertexByIndex(g, v_as_idx);
        // obtiene el vertice del vecino en la lista de vertices
        // a partir de su indice obtenido anteriormente 

        if (Vertex_GetColor(w) == BLACK) {
            time = dfs_recursive_topol(g, w, time);
        }
    }
...
bool dfs( Graph* g, Vertex* start )
{
   ...
   for( size_t i = 0; i < g->len; ++i ){
      ...
      // se usarán en el ordenamiento topológico:
      Vertex_SetDiscovery_time( vertex, 0 );
      Vertex_SetFinish_time( vertex, 0 );
   }

   //dfs_recursive_non_topol( g, start );
   
   // Quitar el comentario para usar
   size_t fT = dfs_recursive_topol(g, start, 0 );
   printf("GRAPH FINISH TIME: %ld \n", fT);
...
int main()
{
   ...
   int key = 1;
   printf( "Trying DFS from %d\n", key );
   dfs( grafo, Graph_GetVertexByKey( grafo, key ) );
